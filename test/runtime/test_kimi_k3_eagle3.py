"""Kimi-K3 EAGLE3 capture and draft-config resolution tests.

These CPU tests pin vLLM's completed-layer capture contract and the exact
target-side tensor used by TokenSpeed's K3 EAGLE3 path.
"""
...
def _vllm_post_layer_attnres_reference(
    prefix_sum: torch.Tensor, hidden_states: torch.Tensor
) -> torch.Tensor:
    """vLLM K3 capture: ``prefix_sum + hidden_states`` after a decoder layer."""
    return prefix_sum + hidden_states


def test_capture_tensor_matches_vllm_attnres_reference():
    """TokenSpeed's post-layer prefix_sum is vLLM's K3 capture tensor."""
    ...
    class FakeLayer:
        ...
        def __call__(self, positions, prefix_sum, ctx, out_cache_loc, block_residual):
            return (
                _vllm_post_layer_attnres_reference(prefix_sum, self.hidden_states),
                block_residual,
            )
    ...
    # vLLM's IDs are one-based completed-layer IDs.  IDs 2 and 3 select the
